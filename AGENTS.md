...
- `packages/core/connection_core/` is the current Connection module boundary. `AppRuntime` creates exactly one `ConnectionDatabase`, `ConnectionRepository`, `CredentialRepository`, and Host Key capability; the database stores no password/private-key columns, and the module uses a new `connection.sqlite` baseline without an old AppDatabase/SharedPreferences migration adapter. The current Connection UI remains on the legacy StorageService compatibility path until the later Feature step.
