...
func NewServer(config Config) *Server {
	...
	return &Server{config: config, hub: newHub(config), upgrader: websocket.Upgrader{CheckOrigin: func(*http.Request) bool { return false }}}
}
...
func (h *hub) routeControl(sender *peer, data []byte) {
	...
	if json.Unmarshal(data, &frame) != nil || len(frame.SessionID) != 32 {
		return
	}
