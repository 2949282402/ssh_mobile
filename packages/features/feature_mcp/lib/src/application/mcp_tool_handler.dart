...
/// MCP tools/list 与 tools/call 的执行层。
///
/// 所有暴露、审批和目标绑定检查都在这里完成；控制台只观察审批队列，不能
/// 绕过 `approval_required` 结果直接调用底层工具。
class McpToolHandler {
  final McpToolExecutor aiToolService;
  ...
  Future<Map<String, dynamic>> _callTool(Map<String, dynamic>? params) async {
    final watch = Stopwatch()..start();
    logger?.info('MCP tools/call called');
    if (params == null) {
  ...
  Future<Map<String, dynamic>> _executeWithSecondaryApproval({
    ...
    try {
      ...
      final approvalGuard = aiToolService is McpApprovalTargetGuard
          ? aiToolService as McpApprovalTargetGuard
          : null;
  ...
  Future<String> _executeWithBinding(
    ...
    final guard = aiToolService is McpApprovalTargetGuard
        ? aiToolService as McpApprovalTargetGuard
        : null;
  ...
  Future<McpApprovalRequest?> _approvalRequestFor(
    String name,
    Map<String, dynamic> arguments,
  ) {
