...
void main() {
  ...
  McpTool tool(
    String name, {
    McpToolExecutionMode mode = McpToolExecutionMode.readOnly,
  }) {
    return McpTool(
      name: name,
      description: name,
  ...
  test(
    ...
    () {
      ...
      for (final value in [
        tool('list_servers'),
        tool('monitor_start', mode: McpToolExecutionMode.stateChanging),
        tool('sftp_delete_entry', mode: McpToolExecutionMode.stateChanging),
      ]) {
        expect(
    ...
    expect(
      policy
          .evaluate(
            tool: tool(
              'monitor_start',
              mode: McpToolExecutionMode.stateChanging,
            ),
            settings: settings,
          )
          .action,
