...
class _PlaybookStrings {
  final PlaybookLanguage language;

  const _PlaybookStrings(this.language);

  bool get _en => language == PlaybookLanguage.en;

  String get title => _en ? 'Playbooks' : '运维剧本与AI编排';
  ...
  String deletePlaybookContent(String name) =>
      _en ? 'Delete "$name"?' : '确定要删除剧本 "$name" 吗？';
  ...
  String deleteStepContent(String name) =>
      _en ? 'Delete "$name"? This cannot be undone.' : '确定删除“$name”吗？此操作不可恢复。';
  ...
  String get expectedRegex => _en ? 'Expected Regex (Optional)' : '预期输出正则 (可选)';

  String get emptyTitle => _en ? 'No Playbooks Yet' : '暂无运维剧本';
  String get emptyHint => _en
      ? 'Create automated playbooks to run sequential SSH commands with AI diagnostic support.'
  ...
  String get selectServerHint =>
      _en ? 'Select a server connection' : '请选择执行该剧本的服务器';

  String get start => _en ? 'Start' : '开始执行';
  String get pause => _en ? 'Pause' : '暂停';
  String get resume => _en ? 'Resume' : '继续执行';
  String get skip => _en ? 'Skip' : '跳过当前步';
  String get reset => _en ? 'Reset' : '重置状态';

  String get aiDiagnostic => _en ? 'AI Diagnostic' : '请求 AI 诊断';
  String get aiSummary => _en ? 'AI Summary' : '请求 AI 总结';
