...
### 1. Component Priority & Agent Workflow
- 组件选型优先级：
  ```text
  app_ui component
        >
...
- Agent 开发默认路径：
  ...
  app_ui components
       ↓
  Design Tokens (Typography / Spacing / StatusColors)
  ```
  严禁 Feature 绕过 `app_ui` 直接自行拼装裸 Material Widget 或自定义私有视觉体系。

### 2. Visual Language & Workspace Standard
- 整体 UI 风格标准：
  - `professional`（专业严谨）
  - `quiet`（克制沉静）
  - `dense`（高信息密度）
...
### 3. Toolbar Policy
- 严禁（Forbidden）：
  - 禁止在 `AppToolbar` 中直接塞入 `IconButton`、`TextButton`、`Container` 等未经封装的组件作为操作按钮。
- 必须遵循（Required）：
  - 必须使用 `AppToolbarAction` 或 `AppToolbarGroup` / `AppToolbarActions` 进行标准化封装。
  - `AppToolbarAction` 具备响应式 Touch Target（移动触控端自动保障 >=44dp 触控热区，桌面端保持 32dp 紧凑尺寸），可使用 `.compact` / `.touch` 显式声明变体。

### 4. Typography Policy
- 严禁（Forbidden）：
  - 严禁在 Feature 内部直接定义 `TextStyle(fontSize: ...)` 魔法字号，导致风格漂移。
- 必须遵循（Required）：
  - 统一使用 `AppTypography` / `context.typography` 令牌：
...
### 5. Spacing Policy
- 严禁（Forbidden）：
  - 严禁在 Feature 内硬编码随机间距数字（如 `EdgeInsets.all(13)`、`SizedBox(height: 17)`、padding 7 等）。
- 必须遵循（Required）：
  - 必须使用统一 `AppSpacing` / `context.spacing` 令牌与预设间距：
    - `xs = 4.0`
    - `sm = 8.0`
    - `md = 12.0`
    - `lg = 16.0`
...
### 8. Toolbar & Navigation Specification
