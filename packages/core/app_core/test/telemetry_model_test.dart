...
void main() {
  group('TelemetryModel', () {
    ...
    test('TelemetrySyncState.fromWireValue round-trips', () {
      expect(TelemetrySyncState.fromWireValue('new'),
          TelemetrySyncState.new_);
      expect(TelemetrySyncState.fromWireValue('pending'),
          TelemetrySyncState.pending);
      expect(TelemetrySyncState.fromWireValue('synced'),
          TelemetrySyncState.synced);
      expect(TelemetrySyncState.fromWireValue('rejected'),
          TelemetrySyncState.rejected);
      // 未知值回退到 pending
      expect(TelemetrySyncState.fromWireValue('unknown'),
          TelemetrySyncState.pending);
    });

    test('TelemetryEventRecord defaults to pending with null logicalDeletedAt',
        () {
