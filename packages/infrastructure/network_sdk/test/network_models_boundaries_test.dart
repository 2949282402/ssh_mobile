...
void main() {
  test(
    'wire enums round-trip supported values and fail closed for unknowns',
    () {
      ...
      expect(
        NetworkRouteTopology.fromWire(-1),
        NetworkRouteTopology.unspecified,
      );
      expect(
  ...
  test(
    'network operations accept every wire name and reject null or empty names',
    () {
  ...
  test(
    'configuration, route, transfer, result, and event models retain values',
    () {
      ...
      expect(
        (events[3] as TransferFailed).error.code,
        NetworkErrorCode.ioError,
      );
      expect((events[4] as IncomingTransferOffer).fileName, 'file.bin');
      expect(
        (events[5] as RouteChanged).snapshot.rtt,
  ...
  test('typed SDK results and disposed exception expose stable state', () {
    ...
    expect(
      const SdkClientDisposedException().toString(),
      'SdkClientDisposedException',
    );
  });
  ...
  test('high-level NetworkFacade delegates every session operation', () async {
    ...
    expect(
      await facade.peerState('peer-a'),
      isA<SdkSuccess<SdkRouteSnapshot>>(),
    );
    expect(
