...
| C | Relay fallback；禁止/失败 direct，使用 WSS Relay | 启动 Relay Compose；`go run ./cmd/relay_smoke -scenario functional -base http://localhost:18080`；native `crypto_handshake::tests::relay_noise_xx_preserves_the_same_session_root` | Relay Route、opaque control/Noise handshake、业务 ACK 与 queue acceptance 分离、文件/消息 exactly-once | native Noise/Relay framing 与 Go Relay quality 已通过；当前 HEAD 的 Docker functional harness 仍需单独复跑 |
...
| O | TCP active；QUIC becomes available and replaces it | `cargo test -p network-core tcp_to_quic_migration_preserves_pending_delivery_and_session_id -- --nocapture` | new QUIC route authenticates before atomic swap, pending Delivery is recovered once, SessionId and the existing Session crypto context remain unchanged, and the old generic carrier is closed after replacement | native integration test passed; E2EE context alias/route-independent crypto unit coverage passed |
| P | Wrong pinned identity or unsupported E2EE capability on QUIC/generic/Relay handshake | `cargo test -p network-core crypto_handshake::tests::noise_xx_rejects_a_wrong_pinned_identity -- --nocapture`; `cargo test -p network-core crypto::tests::missing_session_root_rejects_the_e2ee_default -- --nocapture` | no `CryptoContext`, no Connected Session, and no automatic `CryptoMode::None` fallback | native security tests passed |
