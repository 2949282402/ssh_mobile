...
把当前"进程重启即清空、设备需重新 enroll"的内存状态机，升级为**可持久化、可多实例**
的会话/鉴权/状态架构。

**本轮范围边界**：只做**状态/缓存面**的多实例化。数据面（两个设备之间的 WS 控制帧转发）
保持单实例 + 实例亲和路由，跨实例数据面转发列为后续里程碑（见 §9）。
...
| Q9 | 数据面 | 本期数据面保持单实例，presence 做实例亲和路由；跨实例转发后续里程碑 |
...
| Key | 值 | TTL | 替代 |
|-----|-----|-----|------|
| `presence:{device_id}` | `{instance, last_seen}` | 60s（心跳 20s 续期） | 多实例在线态（替代"读内存 hub.peers"） |
| `nonce:{device_id}:{nonce}` | `1`（原子 SETNX） | 5m | 内存 `proofNonces`（多实例防重放） |
...
- `device.revoked` — 各实例断开该设备连接
- `device.kicked` — 重复登录/管理端踢人，跨实例踢掉旧连接
- `session.invalidate` — admin 会话失效等通用失效通知

### 3.5 核心流程

**鉴权连接（多实例）**
1. 设备连到任意实例 → `upgradeDevice` → `authenticatedRequest`。
2. 实例验 HMAC（共享 `CredentialKey`）+ 过期检查（无状态，各实例独立可验）。
...
**吊销 / 踢人**
- 管理端 revoke → 写 MySQL `revocations` + `devices.status` → 失效缓存 → 发 `device.revoked` → 各实例关闭该设备 peer（跨实例）。
- 重复 deviceID 连接：新连接写 presence 覆盖 → 旧实例检测 presence 归属变化（或 `device.kicked`）→ 关闭旧 peer。
...
| 文件 | 改动 |
|------|------|
| `relay/internal/relay/config.go` | 新增 `DATABASE_URL`、`REDIS_ADDR`/`REDIS_URL`、`REDIS_PASSWORD`、`RELAY_STORAGE_MODE=memory\|mysql` 等字段与 `ConfigFromEnvironment` 解析；确保 `CredentialKey` 由 env 注入供多实例共享 |
...
| `relay/internal/relay/hub.go` | transferSessions 迁 Redis（可选，见 §9）；presence 写入；跨实例踢人 |
| `relay/internal/relay/admin_api.go` | 在线统计改从 Redis presence 聚合（多实例正确） |
...
- **一致性**：写路径统一"先 MySQL → 失效 Redis 读缓存 → 发事件"。不引入分布式事务/锁；enrollment 与吊销以 MySQL 行为准，Redis 读缓存 TTL 兜底防脏。多实例下防重放靠 Redis SETNX 原子性，无竞态窗口。
- **降级（fail-open，针对 Redis）**：Redis 不可用时，鉴权直查 MySQL（跳过读缓存），nonce 防重放降级为进程内尽力而为（接受故障期间多实例重放窗口，日志告警），presence/admin 会话/传输元数据/实时通知降级。核心鉴权不中断。
...
- 多实例部署：各实例共享同一 `CredentialKey`（env）；LB 对同一 deviceID 做**实例亲和（sticky）**路由，配合 presence 提供路由提示，保证数据面控制在同一实例内闭环。
...
5. **Phase 4 — 多实例**：共享 CredentialKey、presence 路由、Pub/Sub 踢人/吊销跨实例生效；压测验证。
...
- **集成**（compose 起 mysql+redis）：重启存活（enroll → 重启 relay → 凭据仍有效，不再需要重新 enroll，**头号行为变化**）；跨实例吊销（实例 A 吊销 → 实例 B 上连接的设备被断开）；Redis 停机回退（停 redis → 鉴权仍可用）；重复登录踢旧连接跨实例。
...
- **本期（M1）**：Phase 0–4 全部落地，交付可持久化、可多实例的会话/鉴权/状态面；数据面保持单实例。
- **后续（M2，显式 deferred）**：跨实例数据面转发（两个设备落在不同实例时控制帧的桥接/路由）——独立工程，需额外设计，等真实多实例流量需要时再启动。
- 传输会话迁 Redis（`transfer:{session_id}`）为**可选**：若短期内传输会话无需跨实例接管，可保留进程内实现，presence 与鉴权先行。
