...
| Flutter Realtime SDK boundary | `network_sdk.RealtimeClient` exposes only `RealtimeSession` lifecycle/state/media contracts; App Shell maps native state events and keeps SDP/ICE/command-result details internal. Queue acceptance is represented by `NetworkRealtimeGateway.NativeCommandTicket`; the App adapter waits for the matching native command result, bounds pending waits, and waits for `closed` before `stopped`. The previous CI baseline passed; current local WSL Flutter tests remain blocked by the `flutter_tester` loopback failure recorded above |
| Generic native transports | `network-transport` TCP/UDP/WebSocket loopbacks and `network-core::connection::GenericConnection` lifecycle/size/backpressure/shutdown tests passed; authenticated TCP and direct WebSocket carriers now enter the composed native Session Route boundary |
| Composed Route fallback | `RouteTopology × RouteTransport` removes flat transport/topology combinations; blocked QUIC selects TCP for `ReliableStream`, reliable Delivery uses authenticated TCP/WebSocket carriers, public events expose composed route metadata, and UDP cannot silently carry reliable delivery |
