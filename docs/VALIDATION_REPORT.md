...
| Rust network workspace | `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --locked --offline -- -D warnings`, and privileged loopback `cargo test --workspace --locked --offline` all passed after Step 4 |
