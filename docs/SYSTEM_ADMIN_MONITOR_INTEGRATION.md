...
本页面记录了 `ssh_mobile` 项目中，将独立的“监控”页合并入“系统管理”控制台后的统一运维体系架构、权限模型及后续开发指南。
...
合并之后，独立的“监控”导航入口已完全删除，由“系统管理”承载全部运维工作。这降低了用户认知成本，减少了约 40% 的冗余代码，并统一了多服务器监控和单服务器管理操作。
...
*   **`selectedConnectionId` (UI 选中服务器 ID)**
    *   **职责**：用于确定“当前选中的服务器”。Monitor、Ports 快照、Applications 快照、Services 快照全部基于此 ID 获取。
    *   **限制**：不要求 root，不要求 Linux，不需要提前建立 root 连接。允许用户选中非 root Linux 或 Windows 机器以只读形式查看快照数据。
*   **`managementConnectionId` (Root SSH 物理连接 ID)**
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Tab 0** | **监控** | 性能图表 / 健康度 / 告警 / 磁盘挂载 | **不要求 root**。支持 Windows/Linux | **是**（多选） | 独立由 `PerformanceMonitorViewModel` 进行全局多服务器采样。 |
