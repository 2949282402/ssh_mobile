...
客户端时钟不可信。服务端必须限制明显异常的未来时间；不得仅依赖 `occurredAt` 做数据保留或最近活跃判断。
...
初始化必须发生在核心业务 Runtime 之前，顺序约束为：

```text
Telemetry local storage
-> existing device identity/deviceId resolution
-> Telemetry Runtime ready
-> Network/SSH/Feature runtime initialization
```

这样 Network Runtime 初始化阶段的错误能够进入 Telemetry。本地 Telemetry 初始化失败不得阻止 App 启动，只能进入本地日志并暴露 degraded 状态。
...
Developer 页面必须提供“一键重传本地 Telemetry 数据”。
...
Policy 故障不得阻止本地采集。`uploadEnabled` 只控制事件上传，不关闭本地记录。Policy 刷新属于独立控制面行为，客户端在 `uploadEnabled=false` 时仍必须以有界频率或启动时刷新 Policy，使管理员能够远程重新开启上传。
...
deviceId
traceId
eventName + occurredAt/receivedAt
...
Analytics Redis 只缓存指定数量的**最新诊断日志摘要**。
...
- Telemetry Runtime 全局唯一且在核心业务 Runtime 前初始化。
...
- `uploadEnabled=false` 时本地采集继续工作且远端 Policy 仍可刷新。
- 人工重传保持原 eventId。
