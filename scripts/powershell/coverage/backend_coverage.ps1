...
try {
  ...
  if (-not $dsn -or -not $redisUrl -or -not $telemetryDsn -or -not $telemetryRedisUrl) {
    ...
    if (-not $dsn) {
      Invoke-CommandChecked docker @('run','-d','--rm','--name',$mysql,'-p','127.0.0.1::3306','-e','MYSQL_ROOT_PASSWORD=root','-e','MYSQL_DATABASE=relay','-e','MYSQL_USER=relay','-e','MYSQL_PASSWORD=relay','mysql:8.4');$started=$true
    ...
    if (-not $redisUrl) {
      Invoke-CommandChecked docker @('run','-d','--rm','--name',$redis,'-p','127.0.0.1::6379','redis:7-alpine');$started=$true
    ...
    if (-not $telemetryDsn) {
      Invoke-CommandChecked docker @('run','-d','--rm','--name',$analyticsMysql,'-p','127.0.0.1::3306','-e',"MYSQL_ROOT_PASSWORD=$analyticsMysqlRootPassword",'-e','MYSQL_DATABASE=telemetry','-e','MYSQL_USER=telemetry','-e',"MYSQL_PASSWORD=$analyticsMysqlPassword",'mysql:8.4');$started=$true
    ...
    if (-not $telemetryRedisUrl) {
      Invoke-CommandChecked docker @('run','-d','--rm','--name',$analyticsRedis,'-p','127.0.0.1::6379','-e',"ANALYTICS_REDIS_PASSWORD=$analyticsRedisPassword",'redis:7-alpine','sh','-ec','exec redis-server --maxmemory 64mb --maxmemory-policy noeviction --requirepass "$ANALYTICS_REDIS_PASSWORD"');$started=$true
