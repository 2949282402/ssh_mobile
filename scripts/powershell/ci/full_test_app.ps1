...
function JobCoverage{$arguments=@('run','tool/check_coverage.dart','--minimum=90');foreach($shard in 0..($AppShards-1)){$coverageDir=Join-Path $LogDir "coverage\shard-$shard";$arguments+="--file=$(Join-Path $coverageDir 'lcov.info')";foreach($name in @('startup','system-admin')){$arguments+="--file=$(Join-Path $coverageDir "isolated-$name-lcov.info")"}};$arguments+='--source-root=lib';$coverageBaseRef=if($env:FULL_TEST_COVERAGE_BASE_REF){$env:FULL_TEST_COVERAGE_BASE_REF}else{$env:GITHUB_EVENT_BEFORE};if($coverageBaseRef){$arguments+="--base-ref=$coverageBaseRef"};Write-Host 'Enforce Full App coverage (90% minimum)';Cmd dart $arguments (Join-Path $root 'apps\ssh_mobile_full')}
