...
function StartStorage{
  ...
  Cmd docker @('run','-d','--rm','--name',$analyticsMysql,'-p','127.0.0.1::3306','-e',"MYSQL_ROOT_PASSWORD=$analyticsMysqlRootPassword",'-e','MYSQL_DATABASE=telemetry','-e','MYSQL_USER=telemetry','-e',"MYSQL_PASSWORD=$analyticsMysqlPassword",'mysql:8.4')
  Cmd docker @('run','-d','--rm','--name',$analyticsRedis,'-p','127.0.0.1::6379','-e',"ANALYTICS_REDIS_PASSWORD=$analyticsRedisPassword",'redis:7-alpine','sh','-ec','exec redis-server --maxmemory 64mb --maxmemory-policy noeviction --requirepass "$ANALYTICS_REDIS_PASSWORD"')
