...
try {
  ...
  Invoke-CommandChecked go @('test', './internal/admin', '-run', '^TestExportAdminAPIContractFixture$', '-count=1') (Join-Path $root 'relay') @{ GOCACHE = $cache; SSH_MOBILE_ADMIN_CONTRACT_FIXTURE = $fixture }
  ...
  Write-Host 'Front ↔ Admin API contract passed.'
} finally {
