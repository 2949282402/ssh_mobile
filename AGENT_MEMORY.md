...
- 2026-06-06: README, `docs/ANDROID_NATIVE_REWRITE_GUIDE.md`, and the shared SSH Mobile maintenance skill were synchronized to the current entry-screen and feature-folder layout. When updating docs, prefer the active `*_screen.dart` entry points and their `part` subfolders over older flat file names.
- 2026-06-09: Implemented system performance optimizations including (1) RAG database partitioning into metadata and document chunk files with background Isolate (`compute()`) offloading for heavy search/BM25/serialization logic; (2) SFTP directory caching with a 30s TTL and SHA-256 keyed preview/thumbnail temp file cache with size/modified-time checking; (3) Performance Monitor sample downsampling for data older than 5 minutes using 10-second grouped averages; (4) Log auto-rotation capped at 5MB per file (max 3 archive files, 20MB total) with an asynchronous log queue. During RAG testing, clean up temporary/partitioned files in `setUp` and `tearDown` to prevent state leaks across test suites.
