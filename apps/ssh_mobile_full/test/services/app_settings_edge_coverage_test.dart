...
void main() {
  ...
  test('loads invalid and legacy persisted values with safe fallbacks', () async {
  ...
  test('all setting APIs preserve state on no-op and persist transitions', () async {
    final settings = AppSettings();
    addTearDown(settings.dispose);
    await settings.ensureCoreLoaded();
    ...
    expect(initialNotifications[0], 0);
    ...
    await settings.setMcpServerHost('127.0.0.1');
    ...
    await settings.setSftpTextPreviewLimitBytes(AppSettings.maxSftpLimitBytes + 1);
  ...
  test('validates relay and MCP input before touching persisted settings', () async {
    final settings = AppSettings();
    addTearDown(settings.dispose);
    await settings.ensureCoreLoaded();
    ...
    for (final endpoint in [
      ...
      await expectLater(settings.setRelayEndpoint(endpoint), throwsArgumentError);
    }
    for (final args in [
      (host: '', port: 443),
    ...
    await expectLater(settings.setMcpServerHost('0.0.0.0'), throwsArgumentError);
