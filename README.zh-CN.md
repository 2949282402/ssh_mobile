...
- `packages/infrastructure/network_sdk/`：Flutter 层 Bootstrap、鉴权 API、Session 和事件流客户端契约，以及不拥有网络资源的 JSON 适配器；`SdkRequestExecutor` 由 App Shell 注入，不拥有 Socket、HTTP client、FFI handle、数据库或 App 生命周期。
