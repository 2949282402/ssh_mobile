...
void _testJsonMatchesYamlSources() {
  ...
  final eventsJson =
      jsonDecode(File('${root.path}/contracts/telemetry/events.json').readAsStringSync())
          as Map<String, dynamic>;
  final errorJson = jsonDecode(
          File('${root.path}/contracts/telemetry/error_codes.json').readAsStringSync())
      as Map<String, dynamic>;
  ...
  _expect(codes.length == contract.errorCodes.length, 'error codes count mismatch');
...
void _testGeneratedDartShape() {
  ...
  _expect(
    eventsDart.contains('class TelemetryEvents {') &&
        eventsDart.contains('static const sshSessionStarted = TelemetryEventDefinition('),
  ...
  _expect(
    eventsDart.contains("name: 'ssh.session.started'") &&
        ...
        eventsDart.contains("allowedProperties: {'auth_method', 'session_type'}") &&
  ...
  _expect(
    errorDart.contains('class TelemetryErrorCodes {') &&
        errorDart.contains('static const sshAuthFailed = TelemetryErrorCodeDefinition('),
...
void _testCheckerDetectsDrift() {
  final root = _repoRoot();
  final eventsDart =
      File('${root.path}/packages/core/app_core/lib/src/telemetry/generated/telemetry_events.dart');
...
void _testCheckerDetectsMissingFile() {
  ...
  try {
    ...
    _expect(checkGenerated(root) == false, 'missing artifact should fail the checker');
  } finally {
    eventsDart.writeAsStringSync(backup);
  }
}
...
void _testIdentifierDerivation() {
  ...
  _expect(
    _errorCodeConstantNameForTest('NET_QUIC_CONN_REFUSED') == 'netQuicConnRefused',
    'error code should map to lowerCamelCase constant',
  );
  _expect(
    _eventConstantNameForTest('lan.discovery.peer_found') == 'lanDiscoveryPeerFound',
...
void _testMalformedYamlRejected() {
  ...
  File('${root.path}/contracts/telemetry/events.yaml')
  ...
  File('${root.path}/contracts/telemetry/error_codes.yaml')
...
String _eventConstantNameForTest(String name) {
  final segments = name.split(RegExp(r'[._]')).where((s) => s.isNotEmpty).toList();
  var result = segments.first.toLowerCase();
  for (final seg in segments.skip(1)) {
    result += seg.isEmpty ? '' : seg[0].toUpperCase() + seg.substring(1).toLowerCase();
  }
