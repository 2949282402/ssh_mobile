...
class IndexAwareCircularBuffer<T extends IndexedItem> {
  ...
  void _adoptChild(int index, T child) {
    ...
    if (child._owner == this) {
      final previousIndex = child.index;
      if (previousIndex != index) {
        _array[_getCyclicIndex(previousIndex)] = null;
      }
    }
    if (_array[cyclicIndex] != child) {
      _array[cyclicIndex]?._detach();
    }
  ...
  void _moveChild(int fromIndex, int toIndex) {
    ...
    if (child == null) {
      ...
      _array[toCyclicIndex] = null;
      return;
    }
    if (_array[toCyclicIndex] != child) {
      _array[toCyclicIndex]?._detach();
    }
    _array[toCyclicIndex] = child.._move(toIndex);
