      ...
      try {
        ...
        if (canRetry) {
          AppLogService.instance.warning(
            ...
            details:
                'endpoint=$endpoint model=${request.model} attempt=${attempt + 1} error=$e',
          );
        ...
        AppLogService.instance.error(
          ...
          stackTrace: stackTrace,
          details:
              'endpoint=$endpoint model=${request.model} attempt=${attempt + 1}',
        );
        rethrow;
      } finally {
        client.close(force: true);
    ...
    for (var attempt = 0; attempt <= 3; attempt++) {
      ...
      try {
        ...
        AppLogService.instance.info(
          ...
          details:
              'elapsedMs=${DateTime.now().difference(startedAt).inMilliseconds} chunks=${contentChunks.length} toolCalls=${finalToolCalls.length} attempt=${attempt + 1}',
        );
