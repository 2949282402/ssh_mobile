    ...
    for (var attempt = 0; attempt <= 3; attempt++) {
      ...
      try {
        ...
        AppLogService.instance.info(
          ...
          details:
              'elapsedMs=${DateTime.now().difference(startedAt).inMilliseconds} chars=${contentText.length} toolCalls=${toolCalls.length} attempt=${attempt + 1}',
        );
    ...
    for (var attempt = 0; attempt <= 3; attempt++) {
      ...
      try {
        ...
        if (response.statusCode < 200 || response.statusCode >= 300) {
          ...
          if (useTools &&
              response.statusCode == 400 &&
              looksLikeToolUnsupportedError(body)) {
            AppLogService.instance.warning(
