...
class ToolSecretPolicy {
  ...
  static final List<RegExp> _secretAssignmentPatterns = [
    RegExp(
      r'\b(password|passwd|pwd|private[_-]?key|api[_-]?key|access[_-]?token|token|secret)\b\s*[:=]\s*([^\s,;}\]]+)',
      caseSensitive: false,
    ),
    RegExp(
      r'\b(password|passwd|pwd|private[_-]?key|api[_-]?key|access[_-]?token|token|secret)\b\s+"([^"]+)"',
      caseSensitive: false,
    ),
    RegExp(
      r"\b(password|passwd|pwd|private[_-]?key|api[_-]?key|access[_-]?token|token|secret)\b\s+'([^']+)'",
  ...
  static final List<RegExp> _suspiciousPathPatterns = [
    ...
    RegExp(r'(^|/)[^/]*(token|secret|api[_-]?key|apikey|credentials)[^/]*$'),
  ];
  ...
  String? suspiciousPathReason(String path) {
    ...
    if (!_looksLikePath(normalized)) return null;
    for (final pattern in _suspiciousPathPatterns) {
