...
class SftpHistoryDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<void> recordVisitedPath({
    ...
    await transaction(() async {
      ...
      final orderedIds = await (selectOnly(sftpRecentPaths)
      ...
      if (staleIds.isNotEmpty) {
        await (delete(sftpRecentPaths)..where((row) => row.id.isIn(staleIds)))
            .go();
      }
    });
  }

  Future<List<SftpRecentPath>> loadRecentPaths(
    String connectionId,
    int limit,
  ) {
  ...
  Future<void> upsertFavoritePath(SftpFavoritePathsCompanion favorite) async {
    await transaction(() async {
      await (delete(sftpFavoritePaths)
            ..where((row) =>
